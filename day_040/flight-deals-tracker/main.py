...
if __name__ == "__main__":
    ...
    wish_data = data_manager.flight_data
    users_data = data_manager.users_data

    """
    wish_data = {
        "prices": [
            {"city": "Paris", "iataCode": "", "lowestPrice": 600, "id": 2},
    ...
    users_data = {
        "users": [
            {"email": "ysying888@gmail.com", "firstName": "Sean", "id": 2, "lastName": "Yang"},
            {"email": "shihying.yang@gmail.com", "firstName": "Shihying", "id": 3, "lastName": "Yang"},
        ]
    }
    """
    ...
    for ind, data in enumerate(wish_data["prices"]):
        ...
        if len(datas) > 0:
            print(f"{len(datas)} flights found for direct flight for {data['city']}")
            ...
            for ind_temp, data_temp in enumerate(datas):
                ...
                if ind_temp == 0:
                    if NOTIF == "sms":
                        ...
                        new_notif = NotificationManager(flight_data, users_data)
                        new_notif.send_emails()
            ...
            print(
                f"No result found for direct flight for {data['city']}. Try to find flights with stopover."
            )
            # add function to find flights with stopover
            new_flight_search_result = flight_search.get_flight_data(stop_over=2)
            new_datas = new_flight_search_result["data"]
            if len(new_datas) > 0:
                print(f"{len(new_datas)} flights found for flights with stop over for {data['city']}")
                for ind_temp2, data_temp2 in enumerate(new_datas):
                    flight_data2 = FlightData(data_temp2, stop_over=2)
                    if ind_temp2 == 0:
                        if NOTIF == "sms":
                            new_notif2 = NotificationManager(flight_data2)
                            new_notif2.send_sms()
                        else:
                            new_notif2 = NotificationManager(flight_data2, users_data)
                            new_notif2.send_emails()
                    with open(
                        f"flight_search_{data['city']}_{today}_indirect.txt", "a", encoding="utf-8"
                    ) as f_in:
                        f_in.write(f"{data_temp2}\n")
            else:
                print(f"No flights found with stop over for {data['city']}")
